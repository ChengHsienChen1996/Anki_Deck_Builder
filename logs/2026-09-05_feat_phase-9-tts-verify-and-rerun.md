...
## 五之二、抽取模型改用 31B q4_K_M（同日追記）

上一節記下「`extract` 慢了一個數量級」之後，使用者裁示改用 Q4_K_M，
並**明確質疑**舊紀錄裡「E4B 優於 31B」的結論——那與「參數量越大越好」的常識相違背。

質疑是對的。兩頁對照實測（同一份 `raw_text`、同一份 prompt，只換 `model:`）：

| | 26B q8 128K | **31B q4_K_M 16K** |
|---|---|---|
| 模型大小 | 29.26 GB | **19.71 GB** |
| 上卡比例 | 70%（30% 在 CPU） | **100%** |
| p11 產出／`reading` 正確 | 14 張／13-16 | 14 張／**13-16**（連錯的都一樣） |
| p11 一頁耗時 | 11.8 分 | **6.7 分** |
| p18 產出／`reading` 忠實度 | 15 張／13-15 | 15 張／**13-15** |
...
2026-08-23 的紀錄是「31B 整頁 94 條目需 40 分鐘、關閉 thinking 後不論切多小
都只回 1 張卡；速度由 34 tok/s 崩到 4~6 tok/s」。那次量到的是**兩個混淆變數**：

1. ComfyUI 常駐 2.4 GB，31B 只載入 85~88% → **被擠出 VRAM**。
   現在 `COMFYUI_FREE_BEFORE_LLM` 必開，實測 **100% 上卡**
2. 當時抽取是**整頁送**的。Phase 8 之後改成分段，超長輸入的症狀消失

也就是說，舊測試量的是「一個被擠出 VRAM 的模型處理一個過大的輸入」，
不是模型本身。**舊實測要標明前提，否則會被當成永久事實而擋住正確的選擇**——
這條紀錄擋了 31B 十三天。

**沒有證明的事**：這次沒有重測 E4B。結論是「31B q4 不劣於 26B q8 且明顯更快」，
不是「參數量越大越好」。26B 也有 Q4_K_M 版本（本機尚未 pull），
那是想兼顧品質與 VRAM 時下一個該試的。

### 兩段式設計又驗證了一次

31B 在對照中唯一輸給 26B 的地方是 `応じる` 的讀音被截斷成 `おう`。
補跑 `verify --pages 11 18` 時，那筆被更正為 `おうじる／おうずる`——
**抽取的缺口由核對接住**，正是兩段式要的效果。

該次核對 14 次呼叫、零失敗、28 筆提案、套用 7，其中 6 筆正確（含 `いりよう→いりょう`
再次被抓到），1 筆改壞：`稲刈り／割稻。` → `稲刈り／稻割。`——模型把**譯文**改成
順著日文語序的詞。`example` 的三道過濾（分隔符、標點、原文行混譯文）都不管譯文用詞，
所以擋不住。單筆樣本，暫不加規則（誤殺風險高於收益），記在此處備查。

### 事後狀態

p11、p18 以 31B 重抽（29 張卡）並補跑 `verify` 與下游四階段，
`pack` 重新產出：**335 張卡、1005 個媒體檔、`deck.zip` 34.2 MB**，七階段零失敗。

---

## 五之三、num_ctx 變體實測，以及媒體清理（同日追記）

使用者推測「KV 採 Q8 量化的話 128K 應該放得下 24 GB」，並隨後建立了 64K 變體。
兩個都實測過，**都不成立**：

| 變體 | 總大小 | 在 VRAM | 上卡 | 生成速度 |
|------|--------|---------|------|----------|
| 16K | 19.71 GB | 19.71 GB | 100% | 31.0 tok/s |
| **32K** | 19.86 GB | 19.86 GB | **100%** | **30.7 tok/s** ← 採用 |
| 64K | 21.50 GB | 18.91 GB | 88% | 12.0 tok/s |
| 128K | 22.78 GB | 17.36 GB | 76% | — |

**12% 的權重掉到 CPU，生成速度就掉 2.6 倍**——部分卸載的代價遠超過比例。
16K 的 31.4 tok/s 正好對上 2026-08-23 舊紀錄裡「完整載入時 34 tok/s」的數字。

三次量測都先查過 `nvidia-smi --query-compute-apps`，確認 GPU 上只有 ollama
自己（22 GB 上下）與一個 370 MB 的 python，**不是被擠掉的**。這一步是刻意做的：
本份日誌前一節才剛批評舊紀錄把「被擠出 VRAM」誤當成「模型太大」。

**KV 是否為 Q8 量化無法確認**（`/proc/<pid>/environ` 權限不足）。但 16K → 128K
只多 3.07 GB、攤到 112K 個 token 相當省，比較像是已有某種壓縮生效
（Gemma 的 sliding window attention 也會大幅縮小 KV），只是模型基礎大小
本來就吃掉太多。

### 懸崖在 32K 與 64K 之間

使用者說明擴大 context 的動機是「未來可能不只是單字，需要盡可能把內容放入」，
於是再測 32K——**它是免費的**：比 16K 只多 0.15 GB、速度差 1%（雜訊範圍內）、
仍 **100% 上卡**。因此採用 32K。

但 64K 只再多 1.64 GB 就掉出 VRAM。**成長不是線性的**
（16K→32K +0.15、32K→64K +1.64、64K→128K +1.28 GB），
所以**不能用內插去猜哪個放得下，必須逐個實測**——我一開始就是用線性內插
估 32K 約 20.16 GB，實際只有 19.86 GB。

目前的單字書工作負載遠用不到 32K（實測 context 用量平均 6676、最大 7579 tokens，
且抽取自 Phase 8 起是分段送的）。選 32K 是為了教材型態改變時不必再回頭調。

### 媒體清理

`work/media` 累積了歷次重跑留下的孤兒檔（`cards.csv` 不再引用）：

- 圖 **286 檔、19.3 MB**（`work/media/img` 621 → 335）
- 音 **572 檔、11.3 MB**（`work/media/audio` 1242 → 670）

刪除後檢查「被引用但檔案不存在」為 0。`work/media` 從 65 MB 降到 37 MB。
`pack` 本來就只收卡片引用到的檔案，所以這只影響工作目錄，不影響產出。
