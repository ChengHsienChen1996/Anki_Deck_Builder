...
class ExtractStage(BaseStage):
    ...
    async def _needs_enrichment(self, row: CardRow) -> bool:
        ...
        ## 為什麼不讓 enrich 常駐（2026-09-01 實測，Phase 8 Task 8.4）

        Phase 8 原本計畫把「例句補全」併進 `EnrichAgent` 並讓它常駐執行。
        **兩項實測都不支持，已放棄：**

        1. **常駐會改動原文。** 拿一頁已有釋義的日語教材跑 enrich，
           條目數、釋義、例句都保住了，但詞性標記被重排成 `[詞性] 名・他サ`、
           釋義被擅自轉繁體、OCR 留下的零散假名行被整批刪掉。
           修掉 prompt 的佔位符 bug 後前兩項改善（詞性標記與日文原文逐字保留），
           **但刪行仍然發生**——它還是會動不該動的東西。
        2. **實產資料上沒有需求。** 掃描 308 張卡，`example` 空的有 **0 張**。
           真正的缺口是「已有例句的分隔格式不合規」（176/308），
           那是確定性的字串正規化，已改由
           `ExtractedCard._normalise_example_separator()` 處理。

        所以 `MaterialTypeAgent` 留著，enrich 維持條件觸發；
        `extract_cards.md` 的〈補全規則〉也留在原處——教材已有釋義時
        enrich 不會跑，補缺口的就是抽取本身。

        要重新考慮的話，先跑一次上述 diff 驗證，不要憑推測改。
        """
