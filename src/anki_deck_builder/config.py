...
class OCRChunkSettings(BaseSettings):
    ...
    #: 頁面轉正方向。**預設 `none`——由使用者負責把輸入影像擺正，不猜。**
    #:
    #: 版面偵測器對頁面方向敏感——實測單字書頁原圖直送偵測到 0 塊、轉正後 28 塊。
    #: 而轉正判錯的代價不只是切線位置：它會讓寬高比反轉 → 文字方向判錯 →
    #: 欄序反了 → **串接出來的文字順序全錯**。
    #:
    #: **2026-09-10 把預設從 `auto` 改成 `none`。** `auto` 在 41 頁實測只對 13 頁
    #: （32%），錯的 28 頁被切成穿過文字行的直條，卡片密度膨脹成正確頁的 1.5 倍
    #: （20.8 vs 13.8 張/頁）並產生 80 張中文拼音卡。當時的評分（文字框聯集面積）
    #: 系統性偏向偵測失敗的一邊，已一併換成框數——詳見
    #: `clients/layout_detector.py` 的 `_best_rotation`。
    #:
    #: 同一批教材的拍攝方向必然一致，**讓使用者說一次比每頁猜一次可靠**。
    #: `auto` 仍可用（評分已修好），但正確性不押在它身上。
    page_rotation: Literal["auto", "none", "cw", "ccw", "180"] = "none"
