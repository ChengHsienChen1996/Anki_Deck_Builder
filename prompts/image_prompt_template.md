...
> ⚠️ **這一節的前提是 SDXL，不是現行的 kyoani / FLUX.2-klein-9B。**
> 2026-09-07 在 FLUX 上重測其中的 `anime illustration`（放在觸發詞位置，
> 併同 `natural anatomy, natural body proportions, coherent pose` 尾綴），
> **重現不出下表的崩壞**——人群場景沒有變成角色特寫，畫風也沒跑掉；
> 但也沒有任何可見的改善。結論是「無害亦無益」，因此現行 prompt 不採用，
> 理由是最小改動而不是它有害。
>
> 下表其餘三項未在 FLUX 上重測，狀態仍是未知。**不要把它們當成永久事實。**
...
## 畸形：治本的是「身體沒有歧義」，不是負向 prompt

**負向 prompt 完全防不了畸形。** 現行 workflow 的 cfg 是 1、負向接
`ConditioningZeroOut`，`.env` 那串 `bad anatomy, bad hands, extra fingers,
deformed…` 一個字都不生效（同〈負向 prompt〉一節）。它留著最大的風險是
讓人以為畸形有被防守。

真正有效的是**在 prompt 端把身體的 configuration 講到沒有歧義**。判準不是
「說得多或少」——2026-09-07 的 A/B 中，有效的那一版平均只比原版長 12%。

### 兩種失敗模式（實測，2026-09-07）

| 失敗模式 | 例 | 生出來的樣子 |
|---|---|---|
| **沒有身體的肢體當主詞** | `A hand pulls back a curtain` | 懸空手臂、手指增生沾黏、多出第三隻手 |
| **身體主軸互相矛盾** | `stretches his arms wide while lying in bed` | 軀幹與手臂軸線打架，姿勢說不通 |

改法是給主體一個身體並釘住數量（`A single person stands beside the window and
pulls the curtain back with one hand`）、只留一個與動作相容的身體主軸
（`sits upright on the bed and stretches both arms upward above his head`）、
並說明四肢方向。

實產牌組 344 張中「無身體肢體當主詞」有 **14 張（4.1%）**；套用新規則後
14/14 全部消失，先前目視確認畸形的五張（`p16_006` 三隻手、`p3_011` 手指增生、
`p1_014` 懸空手、`p2_008` 軸線打架、`p2_007` 手指形狀怪）全部修正。

### 這條規則的副作用

規則強調「主體要有身體」，會讓模型傾向**為每個動作找一個人類施動者**。
場景本身沒有人時會自己補一個——實測 `p17_008 エンジン`（場景是點火鑰匙特寫、
無人物）被改寫成 `A hand turns a car ignition key`，反而製造出新的無身體肢體。
344 張中 1 張（0.3%）。要收掉的話，規則需再補一句：**輸入沒有人物時，
不要為了製造施動者而加入人或肢體。**

### 另一個殘留：語義漂移

去歧義會讓模型重新詮釋動作。`p1_014` 的人物看起來像在**綁**窗簾束帶而不是
**拉開**窗簾——不畸形，但與場景不完全吻合。這是把姿勢講死的代價。
