...
## 核心機制：OCR 分塊輸入（Phase 9）

整頁送 OCR 時**小字會被跳過**：實測同一頁人工可讀出 15 個 furigana，整頁送只回傳 1 個。
全牌組的佐證是卡片的 `reading` 只有 28% 能在該頁 OCR 輸出中找到——
其餘是抽取模型**憑知識生成**的，`あしおと`、`irai` 這類錯誤都是這樣來的。

分塊治本、免費，而且更快。流程全部在 `ocr` 階段**內部**：

```
影像 ──偵測──→ 框（線索，非裁切區）
      ──推結構──→ 文字方向、欄序
      ──完整分割──→ 覆蓋全頁的塊
      ──跳過空白塊──→ 逐塊 OCR ──依序串接──→ raw_text
```

**階段契約不變**（一列影像 → `raw_text`），所以狀態機、CSV schema、CLI、Web UI
全部不動。偵測器依約束 1 走 Protocol，關掉時退回整頁送法。

### 三件反直覺的事

1. **框只能當線索，不能當裁切區。** 框的涵蓋率只有 87%，直接裁會永久遺失
   13% 的文字。切法必須自己推導，且切出來的塊要覆蓋全頁
2. **合併相鄰塊會造成重疊。** 取外接框時會吃進不屬於那兩塊的區域——
   「甲帶的下半」與「乙帶的上半」相鄰，合併它們會同時蓋住「甲帶的上半」
   與「乙帶的下半」。**外接框面積必須等於兩塊之和才合併**
3. **完整覆蓋必然產生空白塊，而空白塊送進 OCR 不是無害的。**
   書溝、頁緣、留白會讓 GLM-OCR 退化成無限重複，一頁吐出 36376 字的垃圾——
   **垃圾比漏字更難清理**。兩道防線：`contains_text()` 只送有文字框落在裡面的塊
   （治本，跳過空白塊不漏內容），`looks_degenerate()` 行重複率 < 0.25 就丟棄

> **偵測器對頁面方向極度敏感**：實測單字書頁原圖直送偵測到 **0 塊**、轉正後 **28 塊**。
> 而自動判斷不可靠（24 頁同一本書分成 15 none／9 cw），判錯又是連鎖的——
> 寬高比反轉 → 文字方向判錯 → 欄序反了 → 串接文字順序全錯。
> 因此轉正方向是**設定**（`OCR_CHUNK_PAGE_ROTATION`），`auto` 只是盡力而為的預設。

---

## 核心機制：核對檢查點（Phase 9）

分塊與去重把源頭修好之後，仍有三類殘留是**確定性規則做不到、但看著影像可以判斷**的：
漏掉的條目、字形錯誤（`いりよう` 的小寫 ょ 變大寫）、去重平手時保留到錯的那一份。

`verify` **不是階段，是獨立子命令**（比照 `pack`）。理由是骨架探測的實測結果：

核對的單位是「一塊影像 → 動好幾張卡」，而 `BaseStage.process_row()` 的契約是
**只改自己那一列**。若照直覺覆寫 `run()` 建立兄弟列索引再就地改動，會**靜默失效**——
`BaseStage.run()` 一開始就 `rows = await store.read()`，而覆寫的 `run()` 為了建索引
也讀了一次，兩次讀出來是**不同的物件**。索引握的是第一批，骨架寫回的是第二批，
結果是沒有例外、沒有警告、`succeeded=3`，而改動一個字都沒進去。

> `image` 與 `audio` 也覆寫 `run()` 重讀一次，但它們只拿來數進度條總數、不改內容，
> 所以這個地雷一直沒被踩到。**要在階段裡改動別的列，就得先動骨架**——那是
> 〈擴充性設計〉明訂要停下來討論的事。

代價是沒有逐列狀態（無續作、Web UI 無單列重跑），換得不動骨架、不動 `STAGE_FIELDS`、
不動 `CardRow` 欄位——加狀態欄位會讓現有 `cards.csv` 因表頭嚴格比對而立刻讀不開。

### 判定管線

模型只回「更正清單」而不是整批卡片（讓它重寫整頁等於再抽取一次）。每一筆提案
逐道過濾，**通過的才套用，被拒絕的一樣寫進稽核檔**：

| 過濾 | 為什麼 |
|------|--------|
| 白名單 `reading`／`example` | `front`／`hint`／`back` 的 26 筆提案僅約 3 筆正確 |
| 一次只問一個範圍 | 專注時抓到的真實錯誤從 5 → 6 → 8 筆 |
| 塊間答案不一致 → 兩筆都不套 | 不一致是免費的信心指標 |
| `example` 分隔符正規化 | 書上的 `原文／譯文` 排版要先還原成專案格式 |
| `example` 只差標點就忽略 | 多數「更正」只差句末的「。」 |
| `example` 原文行混進譯文 → 拒絕 | 第一行是 `tts_back_text` 的來源，會被整句唸出來 |
| `reading` 含括號標註 → 拒絕 | 書上的 `あ（つ）` 會被語音唸出括號 |

`reading` 更正後會**連帶更新 `tts_front_text`**（若它原本等於舊的 `reading`）。
不做這件事的話核對只修好顯示用的讀音，語音仍照著錯的唸——**修了一半比沒修更難發現**。

更正一律寫進 `work/verify-corrections-<時間>.csv`，那是還原的依據，
也是「不自動套用但值得人工挑」的提案唯一的出口。
...
Phase 9 加入第五個 `LayoutDetectorProtocol`——它隔離的是 Phase 1 時還不存在的
外部相依（版面偵測模型），依約束 1 一樣走 Protocol。
...
`LayoutDetectorProtocol`（Phase 9）：
...
class LayoutDetectorProtocol(Protocol):
    def detect(self, image_path: str | Path) -> tuple[list[dict], tuple[int, int]]:
        ...
        框為 {"cls": 類別, "conf": 信心, "box": [x1,y1,x2,y2]}。
        **同步**——底層是 PyTorch 推論，由呼叫端包 to_thread。
        """
```

> **回傳的框不是裁切區域**，是推導版面結構用的線索。直接拿框當裁切區會讓
> 沒偵測到的文字永久遺失（實測框的涵蓋率只有 87%）。切法由
> `stages/ocr_chunking.py` 依這些框推導，**切出來的塊必然覆蓋全頁**。
...
OCR_CHUNK_CONF=0.05
OCR_CHUNK_IMGSZ=1600
```

> `CONF` 與 `IMGSZ` 的預設值是實測掃出來的，**偏一點就失效**：
> conf 0.15 時單字書頁只剩 15 塊、0.3 時剩 4 塊；imgsz 1024 比 1600 少偵到一半。
> 低門檻帶來的誤判由類別白名單擋掉，而漏偵測會直接讓切線消失。
...
| `audio` | ⑥ | VOXCPM2 語音生成 |
| `pack` | ⑦ | 路徑整合與打包 |
| `verify` | — | 對照影像核對卡片。**不是階段**（沒有逐列狀態），跑在 ② 與 ③ 之間 |
| `run-all` | ①–⑦ | 全流程串接。**不含 `verify`**——它沒有逐列狀態，接進去會讓每次執行都重跑全部核對呼叫 |
| `status` | — | 各階段狀態統計 |
| `serve` | — | 啟動本地 Web UI |
| `reset` | — | 重置階段狀態或清空工作檔 |
...
> **媒體格式**：記憶引擎**不限制格式**，判準是瀏覽器播不播得動（2026-08-22 確認）。
> 因此音檔採 `.wav`——`voxcpm` 回傳 float32 波形陣列，`soundfile`（其既有相依）
> 直接寫 WAV，零新增套件；改用 mp3 只會為了省容量而多一個編碼器相依。
> 圖檔同理採 ComfyUI 的原生輸出 `.png`。
