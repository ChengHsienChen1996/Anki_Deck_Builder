...
| 時點 | VRAM | 回報值（2026-08-25，RTX 3090 24 GB） |
|------|------|------|
| audio 開始前（ComfyUI 是否常駐？） | | **522 MiB**（ComfyUI 未啟動，只有桌面） |
| 模型載入後 | | **5995 MiB**（權重約 +5.3 GB） |
| 合成期間峰值 | | **7490 MiB**（推論期再 +1.5 GB） |

**結論：不需要任何讓渡機制。** 峰值 7.5 GB／24 GB，即使 ComfyUI 常駐（約 2.3 GB）
與 E4B 抽取模型（約 8.6 GB）都還在，三者合計約 18 GB 仍有餘裕——與計畫書 §4 的
預估一致。切回 31B 抽取模型（19.87 GB）時才會需要在 audio 前卸載 Ollama。
