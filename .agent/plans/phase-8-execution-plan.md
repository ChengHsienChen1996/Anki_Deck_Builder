...
- 空字串或只有空白 → 失敗
- 含換行、含 Markdown 圍欄、含引號包裹 → 失敗（模型加解說的典型徵狀）
- 超過設定長度上限 → 失敗
...
### Task 8.4 — 條目補全合併進 `EnrichAgent`

**改動**

1. `prompts/enrich_entries.md`：併入 `extract_cards.md`〈補全規則〉的讀音、例句、
   釋義三項；強化「原文已有的一律原樣保留，只補缺的」。
2. `prompts/extract_cards.md`、`extract_cards_vision.md`：刪除〈補全規則〉一節。
3. `stages/extract.py`：`_needs_enrichment()` 改為常駐執行（或依驗證結果保留旁路）。

**驗證（動手前先做，結果決定第 3 點怎麼寫）**

取一頁**已有釋義**的教材，跑 enrich 前後做 diff，確認：
原有釋義、例句、詞性標記逐字保留，只有缺的被補上。
若守不住，`MaterialTypeAgent` 留著、改成「有釋義時只補例句」的窄化模式。

**測試**：enrich 常駐後 extract 的既有測試全綠；
`--enrich/--no-enrich` 旗標語意變更同步更新說明與測試。
