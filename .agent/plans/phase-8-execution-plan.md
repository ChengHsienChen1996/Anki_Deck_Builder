...
### Task 8.5 — A/B 閘門（2026-09-01 **通過**）

完整紀錄：[logs/2026-09-01_eval_prompt-layer-ab.md](../../logs/2026-09-01_eval_prompt-layer-ab.md)

13 張（含四張硬卡），同一批 `image_scene`、同一組 seed、同一份 workflow，
唯一變數是語法層。對照組以程式串接（`Anime. ` ＋ 場景 ＋ 統一風格後綴），
等價於 Phase 7 的實際行為，且零 LLM 變異。

| 判準 | 結果 |
|------|------|
| 語義符合度與構圖 | 自然語言組 **9 勝 1 敗 3 和** |
| 假文字 | **兩組完全相同（2/13）**，兩張都追得到語義層，與語法無關 |
| 觸發詞逐字保留 | **13/13** |

順帶量到語義層本身的改善：同 13 張的禁用道具違規，
舊（extract 順便產）4/13 → 新（`SceneAgent` 專職）**1/13**。

**沒有觸發中止條件**，Task 8.6 可以鋪開。

過程中把語義保底門檻從 0.3 壓到 **0.15**：它在這一輪誤殺了 `p2_016`
（同義替換使重疊率 20%）。詞袋重疊率結構性地分不出「同義改寫」與「換題材」，
16 次真實改寫裡貢獻 2 次誤殺、0 次真陽性，因此定位改為災難偵測器。

---

### Task 8.6 — 全量重跑與打包（2026-09-01 完成）

先還原 308 張英文牌組（`cards.csv.bak-2026-08-26` → 遷移 44 欄 → 清空四個階段狀態）；
現行工作檔那 13 張日文卡連同它們的媒體備份到 `work/backup-japanese-2026-09-01/`。

> **順帶發現的坑**：兩副牌組的 `card_id` 前綴相同（都是 `p1_xxx`）又共用同一個
> `WORK_DIR`，今早那次日文執行把英文牌組的 `p1_001`～`p1_013` 媒體覆蓋掉了。
> 本次全量重生，不受影響；但這是「一個服務綁一個 `WORK_DIR`」那個決定的實際代價。

| 階段 | 結果 |
|------|------|
| `scene` | 311/311，0 失敗 |
| `prompt` | 311/311，0 失敗；**觸發詞 308/308 在開頭** |
| `image` | 311/311，0 失敗 |
| `audio_front`／`audio_back` | 各 311/311，0 失敗 |
| `pack` | 308 張卡、924 個媒體檔 |

`output/deck.zip`：**30.5 MiB**（Phase 7 為 29.4 MiB，+3.7%）。

**禁用道具違規（全量 308 張場景）**：舊（extract 順便產）59/308（19%）
→ 新（`SceneAgent` 專職）**26/308（8%）**。

**假文字實際比例**：隨機抽樣 24 張，**4 張（17%）**。四張的來源分三類——
2 張是 `SceneAgent` 違反禁用表（都用了 `calendar`）、1 張是新框架允許的硬情形
（`boarding pass` 本體就是票券）、1 張是模型在乾淨場景上自發加招牌。

**人工檢查（計畫的驗收項）**：查 ComfyUI history 的 200 筆正向 prompt，
**0 筆在 CSV 裡找不到一字不差的對應**——證明 CSV 的 `image_prompt` 就是
ComfyUI 收到的完整字串，中間沒有任何加工。這是移除 `COMFYUI_PROMPT_PREFIX` 的直接收益。

**四張硬卡**（`p3_084`、`p3_037`、`p3_098`、`p1_046`）全部改善：烤爐與食物清楚、
旅行袋主體明確、走廊散場的動勢清楚；`advertisement` 依新框架第 1 條出路
畫成「路人駐足看牆上海報」，構圖精準，海報上的亂碼字依裁示接受。

**移除了語義保底檢查**：全量跑時它又誤殺 3 張（`among`、`ant`、`bitter`），
累計 5 次誤殺、0 次真陽性。三個缺陷都是詞袋比對的本質問題——同義替換、
短詞被排除（`ant` 三個字母不算內容詞）、詞形變化（`tasting` vs `tastes`）。
五次誤殺全部留成回歸測試。理由寫在 `stages/prompt.py` 常數區。

**下次重生時順手做、但不值得為它單獨重跑的事**：禁用道具表補
`thermometer`（刻度儀器招數字）、`projection`（表裡只有 `screen`），
並強化 `calendar` 那一條——實測它被違反了兩次。
