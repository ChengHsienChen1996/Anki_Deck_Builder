...
英文教材的 **deck 詞性中英混雜**（noun 27／名詞 14／verb 10…）與
**釋義仍用英文**。prompt 已寫明「教材沒有譯文時用繁體中文」，E4B 未遵從——
輸入全英文時它跟著用英文。屬 8B 模型的服從度極限，再靠 prompt 微調邊際效益低。
可能解法：把「釋義語言」提升為顯式的任務參數（同 `領域`、`牌組前綴`），
由 CLI 傳入而非埋在 180 行 prompt 裡。**日文教材（本專案主要目標）不受影響。**
