...
### 2.11 必填欄位為空卻回報成功（2026-08-24）

使用者回報：抽取顯示成功，`pack` 卻以「必填欄位 back 為空」中止。這是本專案的
bug——`extract` 不該把空 `back` 的卡片當成功，`pack` 只是最後一道防線，不該由它
負責發現上游的品質問題。

#### 失敗的形狀：數量對、內容空

```
共 169 張卡，空 back 59 張
空 back 依頁分布: {3: 59}      ← 第 3 頁整頁 59 張全空
p3_001 | front='baby' | deck=English::Vocabulary | example=''
```

那一頁不只 `back` 空，`example` 也空、`deck` 退回英文泛稱——**那幾次呼叫整個
退化了**。但卡片數量正確（59 張對 59 個條目），既有的 `_check_yield` 只數數量，
因此放行。

#### 診斷過程的轉折

初判為文字路徑問題並據此修正，但查資料發現**三列的 `raw_text` 都是 0 字、
`ocr_status=pending`**——實際走的是 `vision_direct`。而新加的檢查裝在文字路徑上，
影像路徑在檢查之前就 return 了。**若未查看實際資料，該修正會完全打空，
且測試仍會全綠**（測試同樣只涵蓋文字路徑）。
...
| 失敗形態 | 檢查 | 補救 |
|----------|------|------|
| 數量不足（7 條目回 1 張） | `_check_yield` | 對半切再試 |
| **數量對、內容空**（59 張全空 `back`） | **`_check_card_quality`** | 文字→對半切；影像→原樣重試一次 |

影像路徑無法切段，只能原樣重試（`MAX_IMAGE_ATTEMPTS = 2`）——實測退化多半是
隨機的，重試一次即可；兩次皆失敗則讓該列失敗，訊息直指「必填欄位為空」。

#### 驗證（使用者實際失敗的 `page_0003.png`）

| | 修正前 | 修正後 |
|---|---|---|
| 卡片 | 59 | 60 |
| 空 `back` | **59** | **0** |
| deck | `English::Vocabulary` | 中文 60／60 |
| 例句 | 0 | 60／60 |
