...
#: 符合 `marks` 條件但**不該自動修**的卡片 → 排除的理由。
#:
#: `p38_002`：`front` 是 `これ腰`，正確詞目是 `腰`（こし）——`これ` 是抽取階段
#: 夾帶的雜訊，`example` 也被污染成 `これ腰が抜ける`。它符合條件只是巧合，
#: 真正的缺陷是詞目本身壞掉。把讀音改成 `これこし` 只會讓 `marks` 歸零而
#: 把真問題藏起來，所以留給人工修 `front`／`example`／`reading` 三個欄位。
#:
#: **這張表變長就是警訊**——代表判定條件選錯了，而不是該繼續往裡面加。
_CARD_SKIP = {
    "p38_002": "front 是 これ腰，正確詞目為 腰（こし）；詞目本身壞掉，非 marks 問題",
}
